%%Modified on 30Jan2019 to match Matlab 2018a lines 6-7 ,82 , 91, 112-113
function [Min, tMin, IMin, Max, tMax, IMax, Npeaks]=fpeaks(t , x, type)
...
%         [PKS1,t1] = findpeaks(x); % maximums of the data
%         if ~isempty(PKS1)
            [PKS1_2,t1_2] = findpeaks(x,'MinPeakProminence',max(x)/2,'Annotate','extents');
            
            % [t1,PKS1] = findpeaks(x); % maximums of the data
            % [t1_2,PKS1_2] = findpeaks([-10000 PKS1 -100000]);
            if strcmp(type, 'data')
            ...
            IdMax = t1_2(a);  % indices of maximums of the data
            ...
            Npeaks =    Ndpeaks;
%         else
%             Min=[];tMin=[]; IMin=[]; Max=[]; tMax=[]; IMax=[]; Npeaks=[];
%         end
